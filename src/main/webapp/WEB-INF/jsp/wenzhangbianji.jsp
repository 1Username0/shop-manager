      ...
      $("#insert").click(function(){
        $("#art").ajaxSubmit({
          success:function(url){
            $.post("saveart",{articleId:$("#articleId").val(),articleTitle:$("#articleTitle").val(),amountOfReading:$("#amountOfReading").val(),articleContent:$("#articleContent").val(),articlePicUrl:url,articleShow:$("input[name='status']:checked").val()})
          }
        });
      })
    })
...
<table width="544"  align="center" cellpadding="3" cellspacing="3" id="art">
  <td><input id="articleId" value="${art.articleId}" style="display:none;"/></td>
  <tr>
  ...
  <tr>
    <td width="87" align="right" >文章内容:</td>
    <td width="421" align="right"><input id="articleContent" type="text" name="Input3" value="${art.articleContent}" /></td>
  </tr>
  <tr>
    <td align="right" valign="top" >图片:</td>
    <td align="right">
      <label id="file_pic">
      <input id="articlePicUrl" type="file" name="1" style="display: none;" />
      <img src="${art.articlePicUrl}" width="400" height="150" >
      </label>
    </td>
  </tr>
  ...
  <tr>
    ...
    <c:choose>
      <c:when test="${art.articleShow==0}">
        <td align="right"><input VALUE="1" type="radio" name="status"  />
          是 |
